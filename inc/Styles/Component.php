...
use function WP_Rig\WP_Rig\sunshine_behavioral_health;
 ...
 * * `sunshine_behavioral_health()->print_styles()`
 */
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `sunshine_behavioral_health()`.
	 *
	...
	public function action_enqueue_styles()
	{
		...
		foreach ($css_files as $handle => $data) {
			...
			$version = sunshine_behavioral_health()->get_asset_version($css_dir . $data['file']);
			...
			if ($data['global'] || !$preloading_styles_enabled && is_callable($data['preload_callback']) && call_user_func($data['preload_callback'])) {
				wp_enqueue_style($handle, $src, [], strtotime(date('Y-m-d H:i:s')), $data['media']);
			} else {
	...
	{
		if ('preconnect' === $relation_type && wp_style_is('sunshine_behavioral_health-fonts', 'queue')) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function ($handle) use ($css_files) {
				...
				if (!$is_valid) {
					...
					_doing_it_wrong(__CLASS__ . '::print_styles()', esc_html(sprintf(__('Invalid theme stylesheet handle: %s', 'sunshine_behavioral_health'), $handle)), 'Sunshine Behavioral Health 2.0.0');
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'sunshine_behavioral_health_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	{
		$preloading_styles_enabled = !sunshine_behavioral_health()->is_amp();
		 ...
		 */
		return apply_filters('sunshine_behavioral_health_preloading_styles_enabled', $preloading_styles_enabled);
	}
	...
	protected function get_css_files(): array
	{
		 ...
		 */
		$css_files = apply_filters('sunshine_behavioral_health_css_files', $css_files);
	...
	protected function get_google_fonts(): array
	{
		 ...
		 */
		$this->google_fonts = (array) apply_filters('sunshine_behavioral_health_google_fonts', $google_fonts);
